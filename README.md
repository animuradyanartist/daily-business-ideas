...
- **Reserve → call → settle, in files.** Every paid request first writes a reservation for its estimated cost (published price × 1.1) to `evidence/budget/holds/<YYYY-MM>/<id>.json`, then calls DataForSEO, then records the provider-reported cost in the same file. A month's committed spend = charged + reserved + uncertain. One file per request, so the workflow's rebase-and-retry push never conflicts on the ledger. The workflow commits `evidence/budget/` even after a failed run.
...
- **Never decides from stale history.** Before a live run buys anything, Scout checks that its checkout's `evidence/budget/` has no uncommitted history and matches the remote branch (`git fetch`); otherwise enrichment stays `pending`. Reservations are serialized by a lock file on one machine and by the workflow's `daily-research` concurrency group in Actions.
- **Possibly-charged failures stay counted.** A timeout, dropped connection, HTTP 5xx without a DataForSEO status, or unreadable reply marks the reservation `uncertain`: it keeps counting at its estimate and the identical request is not re-sent automatically (`--retry-uncertain` after checking the DataForSEO dashboard). A crashed run's reservation turns `uncertain` when it expires. Only the owner resolves it: `node scripts/budget.mjs resolve <id> charged <usd> "<note>"` or `… released "<note>"`, then commit `evidence/budget/`. Only provably unbilled failures (a DataForSEO error status, a request never sent) release a reservation automatically.
...
- **History kept.** Scout's 35 earlier charges ($0.23224, September 2026), recorded while it used the shared database, were exported read-only into `evidence/budget/holds/2026-09/` with their original IDs. `node scripts/budget.mjs status` shows the month.
...
Local-only options: `SCOUT_DFS_ADS_FALLBACK=live` prices keywords Labs has no record of with the Google Ads endpoint ($0.09 per task — off by default; in the first pilot it returned no figures for niche B2B keywords), and `SCOUT_ENRICH_MODELS` overrides the Gemini models used for planning and assessment. A local `--live` run needs a clean, up-to-date checkout; push `evidence/budget/` right after it.
