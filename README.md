# Daily Business Ideas

A daily decision-quality memo from a remote research agent. One idea per day. Bad ideas killed fast.

This system compounds: every run reads `LEARNINGS.md`, `MARKET_MAP.md`, `KILLED.md`, the `markets/` atlas, and `outcomes/` before researching, then writes back to them. Day 30 is sharper than day 1.

## How Scout thinks

Scout doesn't reason in one pass. Each daily run is a 6-stage senior-analyst pipeline, every stage grounded in fresh Google Search and feeding the next:

1. **Scan** — cast wide: 10–12 candidate opportunities across unrelated markets.
2. **Market structure + problem proof** — rigorous TAM/SAM/SOM, growth, fragmentation, margin pool, distribution control, incumbent inertia, plus quantified proof the pain is real (3+ buyer quotes with links, prevalence signals, cost and frequency of the pain).
3. **Economics, moat & reference solutions** — unit economics (pricing, gross margin, CAC/LTV, payback), the defensibility that compounds, and 4–6 real comparable/analogue/precedent solutions that prove the model.
4. **Scope of work** — the honest build: MVP cut, critical-path milestones, load-bearing assumptions, resources required.
5. **Red-team** — a skeptical investor attacks the survivor; every attack is rebutted or conceded.
6. **Synthesis** — the final decision memo with a calibrated 0–100 conviction score that must reconcile with the red-team verdict.

Artifacts it maintains: `ideas/` (daily memos), `markets/` (structural market teardowns + `INDEX.md`), `SCORES.md` (ideas ranked by conviction), `evidence/` (search-demand and competitor evidence for each day's shortlist), plus `LEARNINGS.md` / `MARKET_MAP.md` / `KILLED.md`.

## Evidence enrichment (DataForSEO)

Right after stage 1 shortlists candidates, Scout gathers outside evidence for each of them and writes it to `evidence/<date>.md` (readable) and `evidence/<date>.json` (data + provenance). It never feeds the six stages, so the original memo, score and pick stay exactly as the pipeline produced them. The memo only gains a short "Evidence enrichment" pointer at the end. Enrichment is research, not a build decision: choosing an idea stays manual (⭐ favorites in the bot, results in `outcomes/`).

Per shortlisted idea:

1. **Plan** — problem, target customer and payer, the ONE country + language where the payer searches (chosen from an allowlist, `SCOUT_MARKETS`, and checked against DataForSEO Labs' supported markets), and a small keyword set in three intent groups (problem / solution / buying) written the way people search, plus 1–2 search queries.
2. **Gather** — one DataForSEO Labs `keyword_overview` task per market (volume, 24 months of history, CPC, Google Ads competition), live Google organic results for the queries, and free fetches of ranking sites (homepage + `/pricing`, robots.txt respected with Google's rules). Every observation keeps its URL, retrieval date, market and provider.
3. **Read** — code, not a model, computes the demand and commercial readings. Volumes are never added together, missing data stays "unknown" (never zero), CPC is reported as an advertiser bid (not willingness to pay), ad competition is not treated as SEO difficulty, broad platforms' own pricing (Figma, Miro, Notion…) never counts as proof buyers pay for the idea, and low volume never rejects an idea on its own.
4. **Assess** — a model writes the enriched memo from that evidence only. Code then checks it (`scripts/lib/grounding.mjs`): every citation must carry a verbatim quote found in the cited item — a valid evidence id alone proves nothing; problem evidence cannot rest on keyword rows; "strong" needs two independent domains and "crowded" three vendor-like ones; absence claims are inference; statements about unmeasured or misquoted search demand are removed (and kept for audit); a "what changed" entry must quote Scout's memo (not a heading), cannot be supported or weakened by keywords with no data, cannot validate a paying market from search volume, and cannot "contradict" from something not being seen. The raw model output is stored so the checks can be re-applied offline. There is no numeric confidence score.

### Cost controls — Scout's own spend ledger

Scout accounts for its DataForSEO spend inside this repository. It does not use any other product's database or service (until 2026-09-15 it shared Career OS's Supabase budget; see `specs/scout-budget-separation.md`).

- **Reserve on GitHub → call → settle.** The ledger is the `scout-spend-ledger` branch of this repository: one file per paid request, `holds/<YYYY-MM>/<id>.json`. Before a paid request, Scout commits a reservation for its estimated cost (published price × 1.1) and **pushes it to GitHub**; the request is sent only after GitHub accepts that push. The provider-reported cost is then pushed the same way. A month's committed spend = charged + reserved + uncertain. The job needs `contents: write`.
- **Limits Scout enforces for itself.** A monthly allowance `SCOUT_DFS_MONTHLY_USD_CAP` (no default: unset means nothing is bought), a per-request limit `SCOUT_DFS_MAX_REQUEST_USD` (default $0.10) and a per-run limit `SCOUT_DFS_MAX_RUN_USD` (default $0.10).
- **Not account-wide.** The ledger only knows Scout's own requests. Anything else using the same DataForSEO login (other apps, the dashboard, scripts) is invisible to it and is not limited by it. The only hard limit across the whole account is the prepaid DataForSEO balance.
- **Simultaneous runs cannot spend the same allowance.** Each ledger change is built on the tip Scout just fetched and pushed as a fast-forward. If another run pushed first, GitHub rejects the push; Scout re-reads the ledger, re-checks the allowance including the other run's reservation, and only then retries. A runner that dies after reserving cannot lose the reservation — it is already on GitHub and counts as `uncertain` once expired. GitHub unreachable, the ledger branch missing, or a push refused → nothing is bought.
- **Possibly-charged failures stay counted.** A timeout, dropped connection, HTTP 5xx without a DataForSEO status, or unreadable reply marks the reservation `uncertain`: it keeps counting at its estimate and the identical request is not re-sent automatically (`--retry-uncertain` after checking the DataForSEO dashboard). A crashed run's reservation turns `uncertain` when it expires. Only the owner resolves it: `node scripts/budget.mjs resolve <id> charged <usd> "<note>"` or `… released "<note>"` (pushed to the ledger branch immediately). Only provably unbilled failures (a DataForSEO error status, a request never sent) release a reservation automatically.
- **Failed ledger writes are never lost.** They go to `evidence/ledger-outbox.json`, stop further purchases, and are replayed before anything new is bought.
- **Fails closed.** No allowance, an unreadable ledger file, an unconfirmed ledger, missing credentials, a refused reservation, an unsupported market or a low provider balance → no paid requests; research continues and enrichment is marked `pending` or `unavailable`.
- **Never pays twice.** Keyword data is cached 30 days and search results 14 days in `evidence/cache/` (committed by the workflow, including after a failed run). Enriched ideas are never redone; retries are bounded (`SCOUT_ENRICH_MAX_ATTEMPTS`).
- **Dry run by default.** Nothing is bought unless the repo variable `SCOUT_DFS_MODE` is `live`. A dry run still plans the keywords and shows the projected cost.
- **History kept.** Scout's 35 earlier charges ($0.23224, September 2026), recorded while it used the shared database, were exported read-only onto the ledger branch (`holds/2026-09/`) with their original IDs. `node scripts/budget.mjs status` shows the month as GitHub has it.

### Configuration

Repository **secrets**: `DATAFORSEO_LOGIN`, `DATAFORSEO_PASSWORD` (the API password from the DataForSEO dashboard).
Repository **variables**: `SCOUT_DFS_MODE` (`dry-run` | `live`), `SCOUT_DFS_MONTHLY_USD_CAP` (Scout's own monthly allowance — required for live mode), and optionally `SCOUT_MARKETS` (e.g. `United States:en; India:en; Germany:de`, first = default), `SCOUT_ENRICH_MAX_IDEAS` (default 3, `0` disables), `SCOUT_DFS_MAX_RUN_USD`, `SCOUT_DFS_MAX_REQUEST_USD`.
Local-only options: `SCOUT_DFS_ADS_FALLBACK=live` prices keywords Labs has no record of with the Google Ads endpoint ($0.09 per task — off by default; in the first pilot it returned no figures for niche B2B keywords), and `SCOUT_ENRICH_MODELS` overrides the Gemini models used for planning and assessment. A local `--live` run needs push access to this repository (the ledger branch); `SCOUT_LEDGER_BRANCH` / `SCOUT_LEDGER_REMOTE` override the defaults for testing.

### Pilots and model checks

`scripts/pilot.mjs` runs enrichment on existing ideas without touching `ideas/`, `outcomes/` or the bot's favorites: `select` (explicit queue, else source order) → `plan` → `preview` (dry run) → `gather --live` → `assess` → `revalidate` (offline) → `check` → `report`. Results live in `evidence/pilots/<name>/`.

The model steps run with Scout's own Gemini configuration in `.github/workflows/enrichment-model-check.yml`: manually triggered only (a `model-check:plan` / `model-check:assess` label on a same-repository pull request, or `workflow_dispatch` once the file is on the default branch), `contents: read`, only the Gemini secret, no DataForSEO/budget/Telegram secrets, outputs uploaded as artifacts. `assess` first forces a model failure to prove evidence survives and the next run resumes.

### Commands

```bash
node scripts/enrich.mjs --date 2026-09-13 --dry-run            # plan + projected cost, no paid requests
node scripts/enrich.mjs --date 2026-09-13 --live               # enrich an existing memo (never edits ideas/)
node scripts/enrich.mjs --date 2026-09-13 --plan keywords.json # use a hand-written keyword plan
node scripts/budget.mjs status                                  # Scout's spend this month (Scout only)
node --test scripts/test/*.test.mjs                            # unit + offline pipeline + spend-ledger tests (local git only)
node scripts/pilot.mjs check --pilot evidence/pilots/pilot-10-source-order
```

## Bot

The interactive Telegram interface lives in [`bot/`](bot/). It gives a 3-button menu (📋 ideas / 📈 trends / ⭐ favorites) for browsing this repo's memos directly from Telegram. Runs on Cloudflare Workers, free.

## Log

- 2026-09-24 — The Small Trucking Fleet's "Audit-Ready" IFTA Fuel Tax Hub — 92/100 high
- 2026-09-23 — trend forecast — AI-Automated Towing Compliance (SCRA) +2 more
- 2026-09-23 — The Towing Operator's "Lien-Ready" Impound & Disposal Hub — 95/100 high
- 2026-09-22 — trend forecast — AI-Automated Insurance Claim Documentation for Contractors +2 more
- 2026-09-22 — The Small Business "Penalty-Proof" EPA Tier II Report Generator — 93/100 high
- 2026-09-21 — trend forecast — AI-Powered Electrical Contracting Operations +2 more
- 2026-09-21 — The Small Brewery's "TTB-Ready" Report of Operations Generator — 95/100 high
- 2026-09-20 — trend forecast — AI-Automated Operations for Cleaning Services +2 more
- 2026-09-20 — The Pawn Shop's "Never-Fail" Daily Police Report Generator — 100/100 high
- 2026-09-19 — trend forecast — AI-Automated Home Health Operations +2 more
- 2026-09-19 — The Non-Emergency Medical Transportation (NEMT) Provider's "Get Paid, Not Buried" Hub — 100/100 high
- 2026-09-18 — trend forecast — AI-Automated Firearms Compliance for FFLs +2 more
- 2026-09-18 — The Independent Firearm Dealer's "ATF-Ready" Digital A&D Book — 100/100 high
- 2026-09-17 — trend forecast — AI-Powered Medical Record Retrieval & Analysis for Law Firms +2 more
- 2026-09-17 — The Personal Injury Law Firm's Medical Records Request & Tracking Hub — 100/100 high
- 2026-09-16 — trend forecast — AI-Automated Water Utility Compliance +2 more
- 2026-09-16 — The Small Water Treatment Plant's "State-Ready" Monthly Operating Report (MOR) Generator — 100/100 high
- 2026-09-15 — trend forecast — AI-Powered "Deposition Co-pilot" for Litigators +2 more
- 2026-09-15 — The Agricultural Employer's "Audit-Ready" H-2A Visa Compliance Hub — 100/100 high
- 2026-09-14 — trend forecast — AI Co-pilots for Solo & Small Law Firms +2 more
- 2026-09-14 — The Independent Geotechnical Engineer's "gINT-Replacement" Boring Log Hub — 100/100 high
- 2026-09-13 — trend forecast — AI-Automated Post-Award Grant Management +2 more
- 2026-09-13 — The Small R&D Firm's "Audit-Ready" SBIR/STTR Grant Compliance Ledger — 100/100 high
- 2026-09-12 — trend forecast — AI-Automated SEC Compliance Reporting +2 more
- 2026-09-12 — The Registered Investment Advisor's "Audit-Ready" SEC Marketing Rule Archive — 100/100 high
- 2026-09-11 — trend forecast — AI-Automated Revenue Recovery for B2B Businesses +2 more
- 2026-09-11 — The Independent Insurance Agent's "Leakproof" Commission Reconciler — 100/100 high
- 2026-09-10 — trend forecast — AI-Automated Restaurant Procurement & Inventory +2 more
- 2026-09-10 — The Small Fleet's "Always Compliant" DOT Drug & Alcohol Clearinghouse Hub — 100/100 high
- 2026-09-09 — trend forecast — AI-Powered Veterinary Practice Operations +2 more
- 2026-09-09 — The Veterinary Clinic's "Audit-Ready" DEA Controlled Substance Log — 100/100 high
- 2026-09-08 — trend forecast — AI-Automated Food Safety & Traceability Compliance +2 more
- 2026-09-08 — The Small Machine Shop's "Audit-Ready" Material Traceability Log — 100/100 high
- 2026-09-07 — trend forecast — AI-Automated Operations for Private Therapy Practices +2 more
- 2026-09-07 — The Private Practice Therapist's "Audit-Proof" Note Generator — 99/100 high
- 2026-09-06 — trend forecast — AI-Powered Mobile Co-pilots for Field Technicians +2 more
- 2026-09-06 — The Independent Process Server's "Serve-to-Affidavit" Mobile Workflow App — 98/100 high
- 2026-09-05 — trend forecast — AI-Automated Customs Compliance +2 more
- 2026-09-05 — The US Importer's "ISF 10+2" Filing Preparation Hub — 100/100 high
- 2026-09-04 — trend forecast — AI-Powered NYC Local Law 97 Compliance +2 more
- 2026-09-04 — The NYC Building Owner's "Local Law 97" Compliance Forecaster — 99/100 high
- 2026-09-03 — trend forecast — AI-Powered "Business-in-a-Box" for Home Service Trades +2 more
- 2026-09-03 — The Dental Practice's "Get Paid on Day One" Credentialing Hub — 100/100 high
- 2026-09-02 — trend forecast — AI-Powered Cross-Platform Content Adaptation +2 more
- 2026-09-02 — The "Audit-Ready" Freight Forwarder's Document Hub — 100/100 high
- 2026-09-01 — trend forecast — AI-Automated 401(k) Compliance for SMBs +2 more
- 2026-09-01 — The Small Business's "Penalty-Proof" Form 5500 Filer — 99/100 high
- 2026-08-31 — trend forecast — AI-Automated Pest Control Compliance Documentation +2 more
- 2026-08-31 — The "Audit-Ready" Pesticide Applicator's Log — 100/100 high
- 2026-08-30 — trend forecast — AI-Automated ISO 27001 Compliance +2 more
- 2026-08-30 — The Small Organic Farm's "Audit-Ready" Compliance Ledger — 99/100 high
- 2026-08-29 — trend forecast — AI-Automated Operations for Commercial Cleaning +2 more
- 2026-08-29 — The Construction Subcontractor's "Bid-Ready" Prequalification Packet Generator — 99/100 high
- 2026-08-28 — trend forecast — AI-Automated Lead Response for High-Friction SMBs +2 more
- 2026-08-28 — The Small Law Firm's "Never-Drop-a-Lead" Intake Funnel — 99/100 high
- 2026-08-27 — trend forecast — AI-Powered DOT New Entrant Audit Preparation +2 more
- 2026-08-27 — The Independent Trucking Company's "Audit-Ready" Driver Qualification (DQ) File System — 99/100 high
- 2026-08-26 — trend forecast — AI-Automated Food Safety & Traceability Compliance +2 more
- 2026-08-26 — The "Audit-Ready" Unclaimed Property (Escheatment) Compliance System — 99/100 high
- 2026-08-25 — trend forecast — AI-Automated SOC 2 Compliance +2 more
- 2026-08-25 — The Small SaaS Company's "SOC 2-Ready" Evidence Collector — 98/100 high
- 2026-08-24 — trend forecast — AI-Automated Insurance Applications for SMBs +2 more
- 2026-08-24 — The Self-Storage Operator's Delinquent Unit & Lien Sale Automator — 99/100 high
- 2026-08-23 — trend forecast — AI-Powered Financial Modeling for E-commerce SMBs +2 more
- 2026-08-23 — The Small Machine Shop's "Real-Time Job Profitability" Tracker — 96/100 high
- 2026-08-22 — trend forecast — AI Tutors for High-Stakes Professional Exams +2 more
- 2026-08-22 — The Apprentice Electrician's "Get-My-License" Hour & Category Tracker — 92/100 high
- 2026-08-21 — trend forecast — AI-Automated Equipment Tagging & Tracking for Construction +2 more
- 2026-08-21 — The small brewery's "find my kegs" tracker — 95/100 high
- 2026-08-20 — trend forecast — AI-Powered Cargo Theft Prevention +2 more
- 2026-08-20 — The Freight Broker's "Anti-Double-Brokering" Carrier Verification System — 99/100 high
- 2026-08-19 — trend forecast — AI-Automated Food Supply Chain Traceability +2 more
- 2026-08-19 — The Small Food Producer's "FSMA 204-Ready" Traceability Ledger — 100/100 high
- 2026-08-18 — trend forecast — AI-Powered Operations for Marine Businesses +2 more
- 2026-08-18 — The HR Manager's "I-9 Audit-Ready" Document Collector — 98/100 high
- 2026-08-17 — trend forecast — AI-Automated Government Contracting Compliance +2 more
- 2026-08-17 — The Government Contractor's "CMMC Level 1" Compliance Evidence Locker — 98/100 high
- 2026-08-16 — trend forecast — AI-Powered "Advocate-in-a-Box" for Special Education +2 more
- 2026-08-16 — The special education teacher's "audit-ready" IEP compliance system — 98/100 high
- 2026-08-15 — trend forecast — AI-Automated Home Health Compliance (EVV) +2 more
- 2026-08-15 — The Home Health Agency's "Get Paid by Medicaid" Evidence Locker — 98/100 high
- 2026-08-14 — trend forecast — AI-Powered Lab Management for University Research +2 more
- 2026-08-14 — The University Lab Manager's "Audit-Ready" Chemical Inventory System — 98/100 high
- 2026-08-13 — trend forecast — AI-Powered Pet Symptom Checkers +2 more
- 2026-08-13 — The Real Estate Investor's "1031 Exchange" Deadline Tracker — 98/100 high
- 2026-08-12 — trend forecast — AI-Powered Diagnostic Triage for High-Stakes Exams +2 more
- 2026-08-12 — The NCLEX repeat-taker's "pass-the-second-time" study system — 98/100 high
- 2026-08-11 — trend forecast — AI-Powered Quoting for Home Service Businesses +2 more
- 2026-08-11 — The small job shop's "no-regret" quote generator — 98/100 high
- 2026-08-10 — trend forecast — AI-Powered "Family OS" +2 more
- 2026-08-10 — The subcontractor's "rejection-proof" pay application generator — 98/100 high
- 2026-08-09 — trend forecast — AI-Powered Operations for Independent Hotels +2 more
- 2026-08-09 — The DME supplier's "get paid by Medicare" evidence locker — 98/100 high
- 2026-08-08 — trend forecast — AI-Powered Back Office for Financial Advisors +2 more
- 2026-08-08 — The NEMT provider's "get paid by Medicaid" claim preparer — 98/100 high
- 2026-08-07 — trend forecast — AI-Powered Accessibility Auditing for Digital Products +2 more
- 2026-08-06 — trend forecast — AI-Powered Operations for Used Car Dealerships +2 more
- 2026-08-06 — The independent used car dealer's "recon-to-retail" tracker — 98/100 high
- 2026-08-05 — trend forecast — AI-Automated Food Safety Compliance for Importers +2 more
- 2026-08-05 — The small food importer's "FSVP-ready" compliance binder — 98/100 high
- 2026-08-04 — trend forecast — AI-Guided DIY Auto Repair +2 more
- 2026-08-04 — The independent auto body shop's "get paid by insurance" supplement evidence locker — 98/100 high
- 2026-08-03 — trend forecast — AI-Powered Micro-Influencer Management for SMBs +2 more
- 2026-08-03 — The small-scale cosmetics maker's "MoCRA-ready" compliance system — 98/100 high
- 2026-08-02 — trend forecast — AI-Automated Controlled Substance Compliance for Veterinarians +2 more
- 2026-08-02 — The restaurant manager's "health-inspection-ready" food safety log — 98/100 high
- 2026-08-01 — trend forecast — AI-Automated Operations for Small Landlords +2 more
- 2026-08-01 — The property manager's "security deposit dispute" evidence locker — 98/100 high
- 2026-07-31 — trend forecast — AI-Automated Customs Brokerage Workflows +2 more
- 2026-07-31 — The independent medical courier's "chain of custody" evidence locker — 98/100 high
- 2026-07-30 — trend forecast — AI-Powered Academic Integrity Monitoring +2 more
- 2026-07-30 — The Etsy seller's "true profit" dashboard — 79/100 medium
- 2026-07-29 — trend forecast — AI-Automated Amazon FBA Reimbursement Audits +2 more
- 2026-07-29 — The FBA seller's "get paid by Amazon" reimbursement claim preparer — 95/100 high
- 2026-07-28 — trend forecast — AI-Automated Insurance Claims Documentation for Contractors +2 more
- 2026-07-28 — The water damage restoration contractor's "get paid by insurance" evidence locker — 98/100 high
- 2026-07-27 — trend forecast — AI-Automated Controlled Substance Compliance for Veterinarians +2 more
- 2026-07-27 — The veterinary clinic's "DEA audit-ready" controlled substance log — 98/100 high
- 2026-07-26 — trend forecast — AI-Automated Prime Cost Control for Restaurants +2 more
- 2026-07-26 — The commercial property manager's "tenant COI" tracker — 93/100 high
- 2026-07-25 — trend forecast — AI-Powered Compliance Monitoring for Insurance Agents +2 more
- 2026-07-25 — The independent insurance agency's "E&O audit-ready" file — 98/100 high
- 2026-07-24 — trend forecast — AI-Automated Financial Spreading for Commercial Lending +2 more
- 2026-07-24 — The boutique architecture firm's "submittal & RFI" tracker — 96/100 high
- 2026-07-23 — trend forecast — AI-Automated Food Traceability for Retailer Mandates +2 more
- 2026-07-23 — The small-batch food producer's "lot code & recall readiness" system — 98/100 high
- 2026-07-22 — trend forecast — AI-Automated Operations for Independent Auto Repair Shops +2 more
- 2026-07-22 — The HVAC contractor's "EPA AIM Act-Ready" compliance log — 98/100 high
- 2026-07-21 — trend forecast — AI-Automated Aviation Safety Management Systems (SMS) +2 more
- 2026-07-21 — The Part 135 Air Charter Operator's "FAA Ramp Check-Ready" Digital Binder — 98/100 high
- 2026-07-20 — trend forecast — AI-Automated Evidence Collection for Compliance Audits +2 more
- 2026-07-20 — The General Contractor's "lien waiver" collection & tracking hub — 97/100 high
- 2026-07-19 — trend forecast — AI-Automated Food Safety (HACCP) Documentation +2 more
- 2026-07-19 — The US importer's "ISF (10+2) filing" assistant — 98/100 high
- 2026-07-18 — trend forecast — AI-Powered Clinical Trial Patient Recruitment +2 more
- 2026-07-18 — The small CRO's "FDA audit-ready" eTMF — 98/100 high
- 2026-07-17 — trend forecast — AI-Automated Quality Control for Mortgage Lenders +2 more
- 2026-07-17 — The independent mortgage lender's "loan repurchase" defender hub — 95/100 high
- 2026-07-16 — trend forecast — AI-Automated Customs Brokerage & Trade Compliance +2 more
- 2026-07-16 — The NEMT provider's "audit-proof trip log" to get paid by Medicaid — 98/100 high
- 2026-07-15 — trend forecast — AI-Powered Public Benefits Integrity +2 more
- 2026-07-15 — The freight broker's "carrier compliance" evidence locker — 98/100 high
- 2026-07-14 — trend forecast — AI-Automated Compliance for Aerospace & Defense SMBs +2 more
- 2026-07-14 — The aerospace machine shop's "AS9100 traceability" evidence locker — 98/100 high
- 2026-07-13 — trend forecast — AI-Powered Operations for Auto Repair Shops +2 more
- 2026-07-13 — The special education teacher's "IEP compliance" dashboard — 98/100 high
- 2026-07-12 — trend forecast — AI-Automated Architectural & Construction Plan Review +2 more
- 2026-07-12 — The independent land surveyor's "deed plot" & QA hub — 96/100 high
- 2026-07-11 — trend forecast — AI-Automated Operations for Shopify B2B +2 more
- 2026-07-11 — The high-growth Shopify brand's "wholesale order" portal — 98/100 high
- 2026-07-10 — trend forecast — AI-Automated Agricultural Supply Chain Compliance +2 more
- 2026-07-10 — The Small Farm's "FSMA Audit-Ready" Compliance Locker — 93/100 high
- 2026-07-01 — trend forecast — AI-Automated Post-Award Grant Management +2 more
- 2026-07-01 — The small government contractor's "CMMC audit-ready" evidence locker — 98/100 high
- 2026-06-30 — trend forecast — AI-Automated Commercial Insurance Underwriting +2 more
- 2026-06-30 — The Small Cemetery's "Plot & Records" Management Hub — 94/100 high
- 2026-06-29 — trend forecast — AI-Powered Practice Management for Solo & Small Law Firms +2 more
- 2026-06-29 — The Independent Catastrophe Insurance Adjuster's "Claim File" Builder — 96/100 high
- 2026-06-28 — trend forecast — AI-Automated Amazon FBA Reimbursement +2 more
- 2026-06-28 — The FBA Seller's "Inventory Reconciliation & Reimbursement" Hub — 95/100 high
- 2026-06-27 — trend forecast — AI-Powered Back Office for Small Trucking Fleets +2 more
- 2026-06-27 — The Small Trucking Fleet's "DOT-Audit-Ready" Compliance Hub — 97/100 high
- 2026-06-26 — trend forecast — AI-Automated Construction Payment Workflows +2 more
- 2026-06-26 — The construction subcontractor's "get paid on time" hub — 97/100 high
- 2026-06-25 — trend forecast — AI-Automated Operations for Private Therapy Practices +2 more
- 2026-06-25 — The Independent College Admissions Consultant's "Application Tracker" — 98/100 high
- 2026-06-24 — trend forecast — AI-Automated Immigration Law Workflows +2 more
- 2026-06-24 — The Durable Medical Equipment (DME) Supplier's "Audit-Proof" Delivery Log — 97/100 high
- 2026-06-22 — trend forecast — AI-Automated Childcare Center Operations +2 more
- 2026-06-22 — The Childcare Center's "Stay-in-Ratio" Staffing & Compliance Dashboard — 96/100 high
- 2026-06-21 — trend forecast — AI-Automated Electronic Visit Verification (EVV) Compliance +2 more
- 2026-06-21 — The Home Healthcare Agency's "Audit-Proof" EVV & Scheduling Hub — 96/100 high
- 2026-06-20 — trend forecast — AI-Powered Employee Attrition Prediction +2 more
- 2026-06-20 — The 3PL's "Audit-Proof" Food Safety Compliance Hub — 96/100 high
- 2026-06-19 — trend forecast — AI-Powered Revenue Recovery for B2B +2 more
- 2026-06-19 — The CPG Brand's "Chargeback Defender" Kit — 97/100 high
- 2026-06-18 — trend forecast — AI Co-pilots for Executive Assistants +2 more
- 2026-06-18 — The Small Law Firm's "IOLTA Compliance" Hub — 96/100 high
- 2026-06-17 — trend forecast — AI-Powered Dental Practice Operations +2 more
- 2026-06-17 — The Electrical Contractor's Apprentice Compliance Log — 93/100 high
- 2026-06-16 — trend forecast — AI-Automated Compliance for Aerospace & Defense SMBs +2 more
- 2026-06-16 — The Custom Job Shop's "Audit-Proof" Material Traceability Log — 92/100 high
- 2026-06-15 — trend forecast — AI-Automated Operations for Small Landlords +2 more
- 2026-06-15 — The Veterinary Clinic's "Audit-Proof" DEA Compliance Log — 94/100 high
- 2026-06-14 — trend forecast — AI-Powered Warehouse Management for E-commerce +2 more
- 2026-06-14 — The Clinical Trial Site's "Patient Stipend & Compliance" Hub — 93/100 high
- 2026-06-13 — trend forecast — AI-Powered Scientific Hypothesis Generation +2 more
- 2026-06-13 — The Small Non-Profit's Grant Compliance & Reporting Hub — 95/100 high
- 2026-06-12 — trend forecast — AI-Powered Grant Lifecycle Management for Funders +2 more
- 2026-06-12 — The Academic Research Lab's "Shared Equipment" Scheduler — 89/100 high
- 2026-06-11 — trend forecast — AI-Powered Farm Operations Management +2 more
- 2026-06-11 — The Independent Auto Repair Shop's "Parts Procurement" Hub — 91/100 high
- 2026-06-10 — trend forecast — AI-Powered Accounting & Tax Automation +2 more
- 2026-06-10 — The "Retailer-Ready" Food Traceability Kit for Small Manufacturers — 75/100 medium
- 2026-06-09 — trend forecast — AI-Powered Lone Worker Safety Monitoring +2 more
- 2026-06-09 — The "Proof of Patrol" & Guard Welfare System for Security Companies — 87/100 high
- 2026-06-08 — trend forecast — AI-Automated Property Damage Assessment +2 more
- 2026-06-08 — The Independent Insurance Adjuster's "Claim Narrative" AI Assistant — 88/100 high
- 2026-06-07 — trend forecast — AI-Powered HOA Operations Management +2 more
- 2026-06-07 — The "Know Your Rights & Rules" Hub for Self-Managed HOAs — 86/100 high
- 2026-06-06 — trend forecast — AI-Powered Clinical Trial Patient Recruitment +2 more
- 2026-06-06 — The Local Service Business "Call Source" Tracking Kit — 85/100 high
- 2026-06-05 — trend forecast — AI-Powered Real Estate Brokerage Compliance +2 more
- 2026-06-05 — The Independent Freight Broker's "Carrier Vetting & Compliance" Hub — 84/100 high
- 2026-06-04 — trend forecast — AI-Powered Qualitative Data Analysis +2 more
- 2026-06-04 — The Etsy Seller's "True Profit" Toolkit — 80/100 medium
- 2026-06-03 — **pattern-break memo** — Real Things: buyer-side curated subscription for verified-human-made goods — 67/100 medium
- 2026-06-03 — trend forecast — AI-Powered Wealth Management Automation +2 more
- 2026-06-03 — The Small Construction Co's "Know Your Number" Job Costing Kit — 82/100 high
- 2026-06-02 — trend forecast — AI-Automated Workflows for Real Estate Agents +2 more
- 2026-06-02 — The Shopify Merchant's Real Profit Dashboard — 81/100 medium
- 2026-06-01 — trend forecast — AI-Powered Restaurant Financial Operations +2 more
- 2026-06-01 — The Restaurant Owner's Prime Cost Control Kit — 78/100 medium
- 2026-06-01 — The Independent Restaurant's "Menu Profitability" Kit — high
- 2026-05-31 — trend forecast — AI-Automated Freight Factoring & Cash Flow +2 more
- 2026-05-31 — The Owner-Operator Trucker's "Get Paid Faster" Kit — high
- 2026-05-30 — trend forecast — AI-Powered "Advocate-in-a-Box" for Special Education +2 more
- 2026-05-30 — The Parent's Special Education (IEP/504) Advocacy Kit — high
- 2026-05-29 — trend forecast — AI-Generated Webinar & Workshop Kits +2 more
- 2026-05-29 — The Creator's "First Workshop & Sales Presentation" Kit — high
- 2026-05-28 — trend forecast — AI-Powered Etsy Operations +2 more
- 2026-05-28 — The Etsy Seller's "Perfect Listing" Kit — high
- 2026-05-27 — trend forecast — AI-Automated Client Intake & Qualification +2 more
- 2026-05-27 — The "Creator-to-Coach" First Client Kit — high
- 2026-05-26 — The E-commerce Seller's Supplier Negotiation Kit — high
- 2026-05-25 — trend forecast — AI-Powered Salary Negotiation Coaching +2 more
- 2026-05-25 — The Non-Native Professional's Job Offer Negotiation Kit — high
- 2026-05-24 — trend forecast — AI-Powered Performance Reviews for Managers +2 more
- 2026-05-24 — The Independent Consultant's "Final Report & Value" Kit — high
- 2026-05-23 — trend forecast — AI-Powered Home Renovation Management +2 more
- 2026-05-23 — The Self-Published Author's Podcast Guesting Kit — high
- 2026-05-22 — trend forecast — AI-Automated Podcast Guest Outreach +2 more
- 2026-05-22 — The Non-Native Podcaster's Guest Management Kit — high
- 2026-05-21 — trend forecast — AI-Automated Financial Admin for Solopreneurs +2 more
- 2026-05-21 — The Community Manager's Engagement Rituals Kit — high
- 2026-05-20 — trend forecast — AI-Powered Sales Coaching & Role-Play +2 more
- 2026-05-20 — The Online Coach's High-Ticket Discovery Call Kit — high
- 2026-05-19 — trend forecast — AI-Powered Podcast Production Workflows +2 more
- 2026-05-19 — The Course Creator's Student Retention Kit — high
- 2026-05-18 — trend forecast — AI-Automated E-commerce Customer Service +2 more
- 2026-05-18 — The Etsy Seller's Customer Service Kit — high
- 2026-05-17 — trend forecast — AI-Generated Coloring Books for Digital Marketplaces +2 more
- 2026-05-17 — The Independent Tutor's Parent Communication Kit — high
- 2026-05-16 — trend forecast — AI-Powered Upselling for Freelancers +2 more
- 2026-05-16 — The Freelancer's Project Expansion Kit — high
- 2026-05-15 — trend forecast — AI-Generated Pitch Decks for Fundraising +2 more
- 2026-05-15 — The Freelancer's Revenue-Multiplying Offboarding Kit — high
- 2026-05-14 — trend forecast — AI-Generated Client Onboarding Kits +2 more
- 2026-05-14 — The Freelancer's Project Kickoff Kit — high
- 2026-05-13 — trend forecast — AI-Powered Career Roadmapping +2 more
- 2026-05-13 — The Freelancer's Client-Winning Kit — high
- 2026-05-12 — trend forecast — AI-Powered Go-to-Market Strategy Generation +2 more
- 2026-05-12 — The Solopreneur's Product Launch Kit — high
- 2026-05-11 — trend forecast — Generative AI for E-commerce Product Photography +2 more
- 2026-05-11 — The Design-to-Dev Handoff Kit — high
- 2026-05-10 — trend forecast — AI-Generated Business Proposals & RFP Responses +2 more
- 2026-05-10 — The Design Project Control Kit — high
- 2026-05-09 — trend forecast — AI-Powered Job Application Automation +2 more
- 2026-05-09 — The Research Synthesis & Storytelling Kit — high
- 2026-05-08 — trend forecast — AI-Generated Video for Marketing +2 more
- 2026-05-08 — The User Interview Kit for Non-Native Designers — high
- 2026-05-07 — trend forecast — AI-Powered Design Critique & Validation +2 more
- 2026-05-07 — The Workshop Facilitator's Kit for Non-Native Designers — high
- 2026-05-06 — trend forecast — AI-Powered Communication Coaching +2 more
- 2026-05-06 — The Design Critique Kit for Non-Native Designers — high
- 2026-05-05 — trend forecast — AI-Powered UX Writing & UI Copy Generation +2 more
- 2026-05-05 — The UX Writing Kit for Non-Native Designers — high
- 2026-05-04 — trend forecast — AI-Powered Language Dubbing for Creators +2 more
- 2026-05-04 — The AI Prompt Kit for Non-Native Designers — high
- 2026-05-03 — trend forecast — Spec-Driven AI Development +2 more
- 2026-05-03 — The Figma Handoff Kit for Non-Native Designers — high
- 2026-05-02 — trend forecast — AI-Structured Portfolio Generation +2 more
- 2026-05-02 — The Portfolio Case Study Kit for Non-Native Designers — high
- 2026-05-01 — trend forecast — AI-Powered Conversational Trip Planning +2 more
- 2026-05-01 — The Client Communication Kit for Non-Native Freelance Designers — high
- 2026-04-30 — trend forecast — AI-Assisted UX Research Synthesis +2 more
- 2026-04-30 — The Design Presentation Kit for Non-Native UX/Product Designers — high
- 2026-04-29 — trend forecast — Short-Form Video as an Automated Workflow +2 more
- 2026-04-29 — The UX Research Kit for Non-Native UX Designers — high
- 2026-04-28 — LinkedIn-Native: Canva carousel pack plus native-sounding caption swipes for non-native solo designers — high
- 2026-04-27 — Async English: Slack and PR-comment swipe file for non-native software professionals — high
