// Scout's DataForSEO spend ledger (evidence/budget/) — read it, and resolve uncertain requests.
//
...
//
// `resolve` is the owner's decision after checking the DataForSEO dashboard; commit and push
// evidence/budget/ afterwards so the next run sees it. Only Scout's own requests are listed:
// other users of the same DataForSEO account never appear here.

import { createRepoLedger, readBudgetConfig, LEDGER_DIR } from './lib/spend-ledger.mjs';

const args = process.argv.slice(2);
const cmd = args[0];
const config = readBudgetConfig(process.env);
const ledger = createRepoLedger({ dir: LEDGER_DIR, capUsd: config.capUsd, maxRequestUsd: config.maxRequestUsd });
...
if (cmd === 'status') {
  ...
  const holds = ledger.list(month);
  const sum = (s) => holds.filter((h) => h.status === s).reduce((t, h) => t + (s === 'charged' ? h.actualUsd ?? h.estimatedUsd : h.estimatedUsd), 0);
  const charged = sum('charged');
  const held = sum('reserved') + sum('uncertain');
  console.log(`Scout DataForSEO spend — ${month}`);
  ...
  console.log(`  charged ${money(charged)} (${holds.filter((h) => h.status === 'charged').length}) · held ${money(held)} · remaining ${config.capUsd === null ? 'n/a' : money(Math.max(0, config.capUsd - charged - held))}`);
  for (const h of holds.filter((x) => x.status === 'uncertain' || x.status === 'reserved')) {
    console.log(`  ${h.status.toUpperCase()} ${h.id} ${h.endpoint} est ${money(h.estimatedUsd)} ${h.createdAt}${h.note ? ` — ${h.note}` : ''}`);
  }
  ...
  console.log(`✓ ${holdId} → ${outcome}${usd !== null ? ` ${money(usd)}` : ''}. Commit and push ${LEDGER_DIR}/ so the next run sees it.`);
} else {
