<!-- Final live relevance run 34838611190 (commit adbc017): relevance judgements + assessment from cached evidence, gemini-2.5-pro. This file is that run's own check output. Review: the four broad-keyword ideas are not inflated (the guard caught 'how to write ai prompts' 320/mo judged direct); keyword over-strictness resolved (v3 idea-level low for 7 ideas, matching the person's labels); no level rose. One failure: 2026-04-29 S2.6 was accepted as 'Non-native English-speaking' only through the word 'speak', keeping problem evidence at moderate. Fixed (weak trait words do not identify a trait) and re-applied OFFLINE to this run's stored judgements and raw output — see MODEL-CHECK.md. -->
# Model check — pilot-10-source-order

_Generated 2026-09-14T11:44:01.115Z by `node scripts/pilot.mjs check`. Automated checks; read the evidence file for the full assessments._
...
Assessor: gemini-2.5-pro → gemini-2.5-flash · runner: github-actions run 34838611190

Removed sentences (for review):
- 2026-05-06: "Keywords for the specific solution components like "design feedback scripts" (K5) have no measured search volume."

Relevance cases for a person (not counted as idea evidence):
- 2026-04-27: Keywords with data but unclear relevance (not counted): "how to write professional english" 10/mo, "english for software engineers" 20/mo.
- 2026-04-28: Keywords with data but unclear relevance (not counted): "how to write linkedin posts" 320/mo.
- 2026-04-29: The judge said these items concern the exact target customer, but their text does not mention every defining trait (Non-native English-speaking + UX designer), so they are not counted: S2.9 "User Research Study: Breaking Language Barriers".
- 2026-05-05: "microcopy examples" (110/mo) alone would put idea-level demand at "some"; no other directly relevant keyword is in that band, so it is not counted until a person confirms it is about this idea.
...
| Dollar figures in factual assessment fields match measured CPCs, fetched prices or Scout's memo (proposed experiments excluded) | pass | 1 figure(s) checked |
| Validation actually intervened where the model over-claimed (informational) | pass | 2 citation(s) rejected · 13 level/kind downgrade(s) · 9 claim(s) dropped · 1 sentence(s) removed |
...
| Relevance checks intervened / ambiguous cases flagged for a person (informational) | pass | 5 quoted citation(s) removed as not about the customer and problem · 0 "same customer" judgement(s) narrowed to broader (not every defining trait matched) · 4 flag(s) |
...
_The human review in review.json covers assessment run 34822795160, kept in run.json history; this run (34838611190) has not been reviewed that way._
