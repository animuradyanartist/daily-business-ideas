<!-- Second live assessment run (Actions run 34819658762). Kept as an audit record: its dollar check flagged a proposed pre-order price ($29) inside the experiment design as unverified, and 6 'what changed' claims were dropped because the model elided memo quotes with '...'. Both fixed (factual fields only; ordered-fragment quote matching) before the final run in MODEL-CHECK.md. -->
# Model check — pilot-10-source-order

_Generated 2026-09-14T07:53:06.010Z by `node scripts/pilot.mjs check`. Automated checks; read the evidence file for the full assessments._
...
Assessor: gemini-2.5-pro → gemini-2.5-flash · runner: github-actions run 34819658762
...
| Volume figures quoted for measured keywords match the measurement | pass | 1 figure(s) checked, 0 mismatch(es) |
| Dollar figures in assessments match measured CPCs, fetched prices or Scout's memo (experiment budgets excluded) | **FAIL** | 4 figure(s) checked; unmatched: 2026-04-27: $29 not in pages or memo |
| Validation actually intervened where the model over-claimed (informational) | pass | 3 citation(s) rejected · 1 level/kind downgrade(s) · 8 claim(s) dropped · 0 sentence(s) removed |
