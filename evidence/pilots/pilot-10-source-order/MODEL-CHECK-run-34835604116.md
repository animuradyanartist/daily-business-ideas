<!-- Live relevance run 34835604116 (commit b4552cb). This file is that run's own check output (its trailing human-review section refers to the EARLIER run 34822795160 — a rendering bug fixed since). Review of this run: relevance judgements rejected the r/Journalism 'language barrier' item for 2026-04-29 and no keyword was wrongly judged direct (0 of 40 vs the earlier human labels), but the judge called items about only PART of a two-part target customer 'same customer' (all designers; all non-native speakers), which raised problem evidence for 2026-04-28, 04-29, 04-30 and 05-01. Fixed by requiring every defining trait of the customer to match (effectiveCustomer), pinned by tests from these exact items, and re-run live. -->
# Model check — pilot-10-source-order

_Generated 2026-09-14T11:07:16.745Z by `node scripts/pilot.mjs check`. Automated checks; read the evidence file for the full assessments._
...
Assessor: gemini-2.5-pro → gemini-2.5-flash · runner: github-actions run 34835604116

Relevance cases for a person (not counted as idea evidence):
- 2026-04-29: Items with unclear relevance (not counted): S2.9.
- 2026-05-01: Items with unclear relevance (not counted): P2.
- 2026-05-02: Items with unclear relevance (not counted): P6.
...
| Dollar figures in factual assessment fields match measured CPCs, fetched prices or Scout's memo (proposed experiments excluded) | pass | 3 figure(s) checked |
| Validation actually intervened where the model over-claimed (informational) | pass | 2 citation(s) rejected · 11 level/kind downgrade(s) · 8 claim(s) dropped · 0 sentence(s) removed |
...
| Relevance checks intervened / ambiguous cases flagged for a person (informational) | pass | 2 quoted citation(s) removed as not about the customer and problem · 3 flag(s) |
