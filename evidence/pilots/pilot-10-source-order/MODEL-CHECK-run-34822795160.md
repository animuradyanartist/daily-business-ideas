<!-- Fresh live assessment run (Actions run 34822795160, gemini-2.5-pro for all 4 batches, commit d8745ee validators). This file is that run's own check output. Reviewing every intervention found 3 validator false positives: 2 honest 'has no search volume data' sentences removed, and a 'not only … but also' strength downgraded as an absence claim. The other 8 interventions were correct. The validators were fixed and re-applied OFFLINE to this run's stored raw model output — see MODEL-CHECK.md and REPORT.md. -->
# Model check — pilot-10-source-order

_Generated 2026-09-14T08:31:04.939Z by `node scripts/pilot.mjs check`. Automated checks; read the evidence file for the full assessments._
...
- 2026-04-28: "While the keyword "linkedin content for non native" has no search volume data, search results confirm that non-native speakers on LinkedIn think about their language choices and that language can be a barrier."
- 2026-04-29: "The keyword "user interview language barrier" has no search volume data."
...
| Validation actually intervened where the model over-claimed (informational) | pass | 0 citation(s) rejected · 3 level/kind downgrade(s) · 6 claim(s) dropped · 2 sentence(s) removed |
